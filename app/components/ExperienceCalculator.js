...
export default function ExperienceCalculator() {
    ...
    const calculateTotalExperience = () => {
        const total = experiences.reduce((acc, exp) => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-4">
            <h2 className='text-2xl text-gray-800  font-medium text-center mb-4'>Add Experience</h2>
            ...
            <div className='overflow-x-auto mt-6'>
                <h3 className="text-2xl text-gray-800 font-medium text-center my-4">Experience List</h3>
                <table className="min-w-full bg-white border border-gray-300 shadow-md rounded-lg">
                    <thead>
                        <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                            ...
                            <th className="border border-gray-300 p-2">Total Experience in this company</th>
                    ...
                    <tbody className="text-gray-800 text-sm font-light">
                        {experiences.map((exp, index) => (
                            <tr key={index} className="hover:bg-gray-100">
                                <td className="border border-gray-300 p-2 font-bold">{exp.companyName}</td>
                                <td className="border border-gray-300 p-2">{exp.position}</td>
                                <td className="border border-gray-300 p-2">{new Date(exp.joinDate).toLocaleString('default', { month: 'long', year: 'numeric' })}</td>
                                <td className="border border-gray-300 p-2">{exp.leaveDate === 'Present' ? exp.leaveDate : new Date(exp.leaveDate).toLocaleString('default', { month: 'long', year: 'numeric' })}</td>
                                ...
                                <td className="border border-gray-300 p-2 flex space-x-3">
                                    <button onClick={() => { editExperience(exp.id) }} className="text-blue-500 hover:text-blue-700">
                                        Edit
                                    </button>
                ...
                {experiences.length > 0 && (
                    <>
                        <h3 className='text-2xl text-gray-800  font-medium text-center my-4'>Total Experience Overall:</h3>
